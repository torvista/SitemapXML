...
if ($sniffer->table_exists('TABLE_EZPAGES_TEXT')) {
  $from .= " LEFT JOIN " . TABLE_EZPAGES_TEXT . " pt ON (p.pages_id = pt.pages_id)";
  if ($sniffer->field_exists('TABLE_EZPAGES_TEXT', 'language_id')) {
    $where .= " AND pt.language_id IN (" . $sitemapXML->getLanguagesIDs() . ") ";
  } elseif ($sniffer->field_exists('TABLE_EZPAGES_TEXT', 'languages_id')) {
    $select .= ", pt.languages_id as language_id";
    $where .= " AND pt.languages_id IN (" . $sitemapXML->getLanguagesIDs() . ") ";
  }
}
if ($sniffer->field_exists('TABLE_EZPAGES', 'date_added') && $sniffer->field_exists('TABLE_EZPAGES', 'last_modified')) {
